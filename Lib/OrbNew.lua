...
function Orbwalking:__OnPostAttack()
		for i = 1, #self.OnPostAttackCallbacks do
			self.OnPostAttackCallbacks[i]();
		end
	end

function Orbwalking:OnPostAttack(cb)
	table.insert(self.OnPostAttackCallbacks, cb);
end
...
function Orbwalking:GetHealthPrediction(unit, time, delay)
    ...
    while i <= #self.ActiveAttacks do
        ...
        if self.ActiveAttacks[i].Processed == false and self.ActiveAttacks[i].Source.Addr ~= 0 and not IsDead(self.ActiveAttacks[i].Source.Addr) and self.ActiveAttacks[i].Target and GetIndex(self.ActiveAttacks[i].Target) == unit.NetworkId then
            local landTime = self.ActiveAttacks[i].StartTick + self.ActiveAttacks[i].Delay + math.max(0, GetDistance(self.ActiveAttacks[i].Source.Addr, unit.Addr) - self:GetHitBox(self.ActiveAttacks[i].Source.Addr))
    ...
    while i <= #self.ActiveAttacks do
        ...
        if (GetTimeGame() - 0.1) <= self.ActiveAttacks[i].StartTick + self.ActiveAttacks[i].AnimationTime and self.ActiveAttacks[i].Target --[[and self.ActiveAttacks[i].Target.IsValid]] and GetIndex(self.ActiveAttacks[i].Target) == unit.NetworkId
            ...
            while FromT < ToT do
                ...
                if FromT >= GetTimeGame() and (FromT + (self.ActiveAttacks[i].Delay + math.max(0, GetDistance(self.ActiveAttacks[i].Source.Addr, unit.Addr) - self:GetHitBox(self.ActiveAttacks[i].Source.Addr))
...
function Orbwalking:OnDraw()
    ...
    for i, minions in pairs(self:EnemyMinionsTbl()) do
        if minions ~= nil then
            ...
            --local timeGetHealth = self:WindUpTime() + math.max(0, GetDistance(minion) - self:GetHitBox(myHero.Addr)) / self:GetMyProjectileSpeed() - 0.07
...
function Orbwalking:OnTick()
    ...
    local result = nil
    local mousePos = nil
    if self.menu_keybin_combo.getValue() or self.menu_keybin_laneclearKey.getValue() or self.menu_keybin_harassKey.getValue() or self.menu_keybin_laneclearKey.getValue() then
    ...
    for i, minions in pairs(self:EnemyMinionsTbl()) do
        ...
        --if self.menu_keybin_laneclearKey.getValue() or self.menu_keybin_harassKey.getValue() or self.menu_keybin_lasthitKey.getValue() then
            if minions ~= 0 and GetType(minions) == 1 then
                --local time1 = self:WindUpTime() - 0.1 + self:GamePing() + math.max(0, GetDistance(minion, myHero) - self:GetHitBox(minion.Addr))  / self:GetMyProjectileSpeed() - 0.07
                --local time = self:WindUpTime() - 0.1 + self:GamePing() + math.max(0, GetDistance(minion, myHero) - self:GetHitBox(minion.Addr)) / self:GetMyProjectileSpeed()
                --local timeGetHealth = self:WindUpTime() + self:GamePing() + math.max(0, GetDistance(minion) - self:GetHitBox(myHero.Addr)) / self:GetMyProjectileSpeed() - 0.07
                ...
                for i, barrels in pairs(pUnit) do
                    if barrels ~= 0 then
                        ...
                        if  GetObjName(barrel.Addr) == "Barrel" and GetTeamId(barrel.Addr) == 300 and IsValidTarget(barrel.Addr, 1000) and self:InAutoAttackRange(barrel.Addr) then
                            ...
                            --__PrintTextGame(string szText)
                            local t = self:WindUpTime() + math.max(0, GetDistance(barrel.Addr, myHero) - self:GetHitBox(myHero.Addr))  / self:GetMyProjectileSpeed()
...
function Orbwalking:UnderTurretMode()
    ...
    for i,minions in ipairs(self:turretMinions()) do
        ...
        if turretMinion ~= nil then
            ...
            local turretLandTick = turretStarTick + self:getAttackCastDelay(closestTower) + (math.max(0, GetDistance(turretMinion.Addr, closestTower) - closestTower.CollisionRadius) / (self:GetProjectileSpeed(closestTower)))
