...
function Orbwalking:OnTick()
    ...
    if self.menu_keybin_combo.getValue() or self.menu_keybin_laneclearKey.getValue() or self.menu_keybin_harassKey.getValue() or self.menu_keybin_lasthitKey.getValue() then
    	local target = self:GetTargetOrb()
        --__PrintTextGame(tostring(self.menu_keybin_combo.getValue()))
        --local target = self:GetTarget() --KillableMinion() or UnderTurretFarming() -- GetTargetLS() --GetTarget(1000)
        local mousePos = GetMousePos() --Vector({GetCursorPosX(), GetCursorPosY(), GetCursorPosZ()})
        --if target ~= nil then
            --if GetKeyPress(SpaceKeyCode) == 1 then
        self:OrbWalk(target, mousePos)
        --end
    end

    --[[if self.menu_keybin_combo.getValue() then
    --if self.menu_keybin_combo.getValue() then --and not self.menu_keybin_lasthitKey.getValue() then
        --if not self.menu_keybin_laneclearKey then --or not self:ShouldWait() then
            ...
            --if target ~= nil then --and self:InAutoAttackRange(target) then
            	__PrintTextGame(GetChampName(target))
...
function Orbwalking:OnPlayAnimation(unit, anim)
    if unit.IsMe and myHero.CharName == "Graves" then
        ...
        if anim == "Spell3" or anim == "Spell3withReload" then
            ...
            --__PrintTextGame(tostring(self.LastAATick))
        end
    end
end
...
function Orbwalking:CanMove()
    --if myHero.CharName == "Graves" then
        --local attackDelay = 0.07 --1.0740296828 * self.AnimationTimeOrb() - 0.7162381256175 - 0.08
        --__PrintTextGame(tostring(self:GameTimeTickCount() + self:GamePing()).."<-->"..tostring(self.LastAATick + self:WindUpTime() + self.menu_advanced_delayWindup.getValue() /1000))
        --return self:GameTimeTickCount() + self:GamePing() + 1 > self.LastAATick + self:WindUpTime() + self.menu_advanced_delayWindup.getValue() /1000
    --end
    --extraWindup = extraWindup and extraWindup or 80
    --if self.LastAATick <= self:GameTimeTickCount() then
        --return (self:GameTimeTickCount() + self:GamePing() > self.LastAATick + self:WindUpTime())
        --return CanMove() --or (self:GameTimeTickCount() + self:GamePing() > self.LastAATick + self:WindUpTime())
        return myHero.CharName == "Kalista" or (self:GameTimeTickCount() + self:GamePing() > self.LastAATick + self:WindUpTime() + self.menu_advanced_delayWindup.getValue() /1000)
    --end  
    --return false
    --return CanMove()
end
...
function Orbwalking:InAutoAttackRange(target)
    ...
    return GetDistance(myHero, target) < GetTrueAttackRange()
end
...
function Orbwalking:OrbWalk1(target, position, extraWindup, holdAreaRadius, useFixedDistance, randomizeMinDistance)
    ...
    if IsValidTarget(target, 1000) and self:CanAttack() and not self:BeforeAttack(target) then
...
function Orbwalking:OrbWalk(target, point)
    ...
    elseif self:CanMove() and self.Move then
        if not point then
            ...
            local targetPos = Vector({GetPosX(myHero.Addr), GetPosY(myHero.Addr), GetPosZ(myHero.Addr)})
            local targetPos = Vector({GetPosX(target), GetPosY(target), GetPosZ(target)})

            if self.menu_keybin_harassKey.getValue() or not OBTarget then
                local Mv = myHeroPos + 400 * (mousePos - myHeroPos):Normalized()
                MoveToPos(Mv.x, Mv.z)
            elseif self:GetDistanceSqr(OBTarget) > 100*100 + math.pow(self:GetHitBox(OBTarget), 2) then
                local point = targetPos

                if self:GetDistanceSqr(point) < 100*100 + math.pow(self:GetHitBox(OBTarget), 2) then
...
function Orbwalking:GetHealthPrediction(unit, time, delay)
    ...
    while i <= #self.ActiveAttacks do
        ...
        if self.ActiveAttacks[i].Processed == false and self.ActiveAttacks[i].Source.Addr ~= 0 and not IsDead(self.ActiveAttacks[i].Source.Addr) and self.ActiveAttacks[i].Target and GetIndex(self.ActiveAttacks[i].Target) == unit.NetworkId then     
            local landTime = self.ActiveAttacks[i].StartTick + self.ActiveAttacks[i].Delay + math.max(0, GetDistance(self.ActiveAttacks[i].Source, unit) - self:GetHitBox(self.ActiveAttacks[i].Source.Addr))
...
function Orbwalking:LaneClearHealthPrediction(unit, t)
    ...
    while i <= #self.ActiveAttacks do
        ...
        if (GetTimeGame() - 0.1) <= self.ActiveAttacks[i].StartTick + self.ActiveAttacks[i].AnimationTime and self.ActiveAttacks[i].Target --[[and self.ActiveAttacks[i].Target.IsValid]] and GetIndex(self.ActiveAttacks[i].Target) == unit.NetworkId 
            ...
            while FromT < ToT do
                --__PrintTextGame(tostring(GetDistance(self.ActiveAttacks[i].Source, unit) - self:GetHitBox(self.ActiveAttacks[i].Source.Addr)))
                if FromT >= GetTimeGame() and (FromT + (self.ActiveAttacks[i].Delay + math.max(0, GetDistance(self.ActiveAttacks[i].Source, unit) - self:GetHitBox(self.ActiveAttacks[i].Source.Addr))
...
--[[function Orbwalking:GetTargetById(targetid)
...
function Orbwalking:OnProcessSpell(unit, spell)
    ...
    if unit.IsMe and self:IsAutoAttack(spellName)  then 
        ...
        --self:AfterAttack(GetTargetById(spell.TargetId))
    ...
    if --[[IsValidTarget(unit.Addr, 3000) and]] unit.TeamId == myHero.TeamId --[[and unit.Type ~= myHero.Type]] 
        and GetType(GetTargetById(spell.TargetId)) == 1 
        and spellName:find("attack") ~= 0 
        and (unit.Type == 0 or unit.Type == 1 or unit.Type == 2) 
    ...
    ---------------------- L# ------------------------
                ...
                local time = GetTimeGame() + GetDistance(GetTargetById(spell.TargetId), unit) / spell.MissileSpeed - self:GamePing()


                local i = 1
                while i <= #self.ActiveAttacks do
                    if (self.ActiveAttacks[i].Source.Addr ~= 0 and self.ActiveAttacks[i].Source.NetworkId == unit.NetworkId) then -- or self.ActiveAttacks[i].Hittime + 3 < GetTimeGame() then
                        table.remove(self.ActiveAttacks, i)                        
                    end
                    i = i + 1
                end

                --__PrintTextGame(tostring(spell.Delay).."<->"..tostring(WindUpTime()).."<->"..tostring(GetCDBA(unit.Addr) - aniTime).."<->"..tostring(AnimationTimeOrb()))--.."<->"..tostring(GetChampName(GetTargetById(spell.TargetId))))
...
function Orbwalking:OnDoCast(unit, spell)
    if unit.Type == 1 then
        local minon = GetUnit(unit.Addr)
        local i = 1
        while i <= #self.ActiveAttacks do
            if self.ActiveAttacks[i].Source.NetworkId == minon.NetworkId and minon.IsMelee then
...
function Orbwalking:OnDraw()
        ...
        for i, minions in pairs(self:EnemyMinionsTbl()) do
        	if minions ~= 0 then
	            ...
	            if  IsEnemy(minions) and not IsDead(minions) and not IsInFog(minions) and (GetType(minions) == 1 or GetType(minions) == 2) then                                    
	                local time1 = self:WindUpTime() + math.max(0, GetDistance(minion, myHero) - self:GetHitBox(minion.Addr))  / self:GetMyProjectileSpeed() - 0.07
	                local time = self:WindUpTime() - 0.1 + self:GamePing() + math.max(0, GetDistance(minion, myHero) - self:GetHitBox(minion.Addr)) / self:GetMyProjectileSpeed()
    ...
    --[[for i, minions in pairs(self:EnemyMinionsTbl()) do
    	--__PrintTextGame(tostring(minions))
        if minions ~= nil then
        	__PrintTextGame(tostring(minions))
            local minion = GetUnit(minions)

            --local time = WindUpTime() - 0.1 + GetLatency() / 2000 + math.max(0, GetDistance(minion) - myHero.CollisionRadius) / GetMyProjectileSpeed()
            local timeGetHealth = self:WindUpTime() - 0.1 + self:GamePing() + math.max(0, GetDistance(minion) - myHero.CollisionRadius) / self:GetMyProjectileSpeed()
            --local timeLaneClear = self:AnimationTimeOrb() + math.max(0, GetDistance(minion) - myHero.CollisionRadius) / self:GetMyProjectileSpeed()

            --local time1 = self:WindUpTime() -0.1 + math.max(0, GetDistance(minion, myHero) - self:GetHitBox(minion.Addr))  / self:GetMyProjectileSpeed() - 0.07
    ...
    for i, minions in pairs(self:EnemyMinionsTbl()) do
        ...
        if self.menu_keybin_laneclearKey.getValue() or self.menu_keybin_harassKey.getValue() or self.menu_keybin_lasthitKey.getValue() then
            if minions ~= 0  and IsEnemy(minions) and not IsDead(minions) and not IsInFog(minions) and (GetType(minions) == 1 or GetType(minions) == 2) then    
                ...
                --local time1 = self:WindUpTime() + math.max(0, GetDistance(minion, myHero) - self:GetHitBox(minion.Addr))  / self:GetMyProjectileSpeed() - 0.07
                --local time = self:WindUpTime() - 0.1 + self:GamePing() + math.max(0, GetDistance(minion, myHero) - self:GetHitBox(minion.Addr)) / self:GetMyProjectileSpeed()
                local timeGetHealth = self:WindUpTime() - 0.1 + self:GamePing() + math.max(0, GetDistance(minion) - self:GetHitBox(myHero.Addr)) / self:GetMyProjectileSpeed()
                ...
                for i, barrels in pairs(pUnit) do
                    if barrels ~= 0 then
                        ...
                        if  GetObjName(barrel.Addr) == "Barrel" and GetTeamId(barrel.Addr) == 300 and IsValidTarget(barrel.Addr, 1000) and self:InAutoAttackRange(barrel.Addr) then
                            ...
                            --__PrintTextGame(string szText)
                            local t = self:WindUpTime() + math.max(0, GetDistance(barrel, myHero) - self:GetHitBox(myHero.Addr))  / self:GetMyProjectileSpeed()
...
function Orbwalking:AttackWard()
    ...
    for i, wards in pairs(pUnit) do
    	if wards ~= 0 then
    		...
    		and IsValidTarget(ward.Addr, 1000) and self:InAutoAttackRange(ward.Addr) then            
	            return ward.Addr
	        end
...
function Orbwalking:UnderTurretFarming()
    ...
    for i, turrent in ipairs(self:TurrentAlly()) do    
        if turrent ~= 0 and GetDistance(turrent) < 1500 then
    ...
    for i,minions in ipairs(self:turretMinions()) do
        ...
        if turretMinion ~= nil then
            ...
            local turretLandTick = turretStarTick + self:getAttackCastDelay(closestTower) + (math.max(0, GetDistance(turretMinion, closestTower) - closestTower.CollisionRadius) / (self:GetProjectileSpeed(closestTower)))

            for i= turretLandTick, turretLandTick + 3 * self:getAttackDelay(closestTower.Addr), self:getAttackDelay(closestTower.Addr) do                           
            ...
            if hpLeft == 0 and turretAttackCount ~= 0 and hpLeftBeforeDie ~= 0 then 
                ...
                if self.LastAATick + self:AnimationTimeOrb() > self:GameTimeTickCount() then --+ self:GamePing() then
                    timeUntilAttackReady = self.LastAATick + self:AnimationTimeOrb() - self:GameTimeTickCount()-- + self:GamePing()
                else
                ...
                if GetTrueAttackRange() < 300 then
                    ...
                    timeToLandAttack = self:WindUpTime() + math.max(GetDistance(turretMinion) - myHero.CollisionRadius) / self:GetProjectileSpeed(myHero)
                end
            ...
            if minion ~= 0 then
                ...
                if minions.Type == 1 then
                    if IsValidTarget(minions.Addr, 1000) 
                        ...
                        and GetDistance(minions, closestTower) < 900 then
...
function Orbwalking:ShouldWaitUnderTurret(noneKillableMinion)
    ...
    for i, minions in pairs(pUnit) do             
        if minions ~= 0 then
            ...
            local minion = GetUnit(minions)  
            if noneKillableMinion ~= nil and GetIndex(noneKillableMinion) ~= GetIndex(minion.Addr) and IsValidTarget(minion.Addr, 1000) and GetTeamId(minion.Addr) ~= 300 and self:InAutoAttackRange(minion.Addr) then
